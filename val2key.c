...
int main (int argc, char *argv[]){
	char baseFileName[100]; //change to actual max file name (title.principals.kv) 
	int capacity; 
	int hash_val; 
	int hash_val_start; 
	char holder[STRLEN]; 
	...
	strncat (baseFileName, ".vhs\0", 5);
	
	fp = fopen (baseFileName, "r"); //getting capacity for hash file as determined by user input with buildidx 
	capacity = get_capacity (fp); 
	fclose (fp); 
	
	fp = fopen (baseFileName, "r"); //opening khs file to read index stored at hashed value 
	fp2 = fopen (argv[1], "r"); //opening kv file to read index after retrieving it from the stored location in the vhs file (hashed index location)
	hash_val = hashfn (argv[2], capacity); //initial hash for starting point
	hash_val_start = hash_val; //setting a variable to keep track of initial hash val to know when to stop searching 
	while (hash_val != hash_val_start || match < 1){ //hash_val does equal hash_val_start for first iteration but match will be less than 1 (0) 
		...
		if (ret_index == -1){ //if the index stored is empty, can't compare, so increment hash index and continue 
			hash_val++; 
		}else{ //if not empty, compare search value with value extracted from kv file 
			read_val (fp2, ret_index, ret_val); //use the index stored at the hash value to retrieve the key in the kv file for comparison
			if (strcmp (ret_val, argv[2]) == 0){ //if the key in the kv file matches the searched key then read the value corresponding to that key and print it  
				read_key (fp2, ret_index, ret_key); 
				if (strcmp (holder, ret_key) != 0){
					printf ("%s\n", ret_key);
				} 
				strcpy (holder, ret_key); 
				hits++; 
			}
